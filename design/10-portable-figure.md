...
Status: **P0 shipped** in 1.7.0; **P1 shipped** in 1.8.0 — the renderer-free
`portable::inspect`, the `PSTR` poster, schema 2's `meta` block, a published
hash-addressed runtime, `WasmFigure::from_portable`, and `rizzma-mount.js`;
...
```rust
// `rizzma::portable`, feature `portable`: builds no figure, renders nothing.
...
| **P1** ✅ | the four host prerequisites below, then `WasmFigure::from_portable` + `rizzma-mount.js`; linked profile end-to-end | 2 |
...
2. **`inspect()` is a feature of `rizzma`**, `portable`, not a separate crate.
   ...
   `inspect` comes to **~22 KB**, because dead-code elimination strips the
   renderer and the embedded font when nothing references them. What a separate
   crate actually bought was a smaller dependency *tree* — build time and audit
   surface, roughly 13 lines of `cargo tree` versus 45 — not a smaller shipped
   artifact, which is what the argument had claimed. Not worth a second crate to
   publish, version, and keep in sync, so it lives here and the container
   framing plus the shared metadata types are public alongside it.
