...
impl<'a> Parser<'a> {
    ...
    fn parse_command(&mut self) -> Node {
        ...
        if name == "operatornamewithlimits" {
            return self.parse_operatorname(start, name);
        }
    ...
    fn parse_operatorname(&mut self, start: usize, command: &str) -> Node {
        self.parse_roman_text_command(start, command)
    }
...
fn command_symbol(name: &str) -> Option<&'static str> {
    match name {
        ...
        "leftleftarrows" => Some("⇇"),
        "rightrightarrows" => Some("⇉"),
        "leftrightarrows" => Some("⇆"),
        "rightleftarrows" => Some("⇄"),
        "upuparrows" => Some("⇈"),
        "downdownarrows" => Some("⇊"),
        "updownarrows" => Some("⇅"),
        "downuparrows" => Some("⇵"),
...
mod tests {
    ...
    #[test]
    fn comparison_symbol_aliases_map_to_covered_unicode_glyphs() {
        let font = font();
        for ch in ['≲', '≳', '⪅', '⪆', '≶', '≷'] {
        ...
        let layout = layout_math(
            "\\lesssim\\gtrsim\\lessapprox\\gtrapprox\\lessgtr\\gtrless",
            &font,
            20.0,
        );
        let text: String = layout
    ...
    #[test]
    fn negated_turnstile_aliases_map_to_covered_unicode_glyphs() {
        let font = font();
        for ch in ['⊬', '⊭', '⊮', '⊯'] {
            assert!(font.has_glyph(ch), "expected DejaVu coverage for {ch}");
        }
    ...
    #[test]
    fn negated_divisibility_aliases_map_to_covered_unicode_glyphs() {
        let font = font();
        for ch in ['∤', '∦'] {
            assert!(font.has_glyph(ch), "expected DejaVu coverage for {ch}");
        }
    ...
    #[test]
    fn negated_similarity_aliases_map_to_covered_unicode_glyphs() {
        let font = font();
        for ch in ['≁', '≉', '≇'] {
            assert!(font.has_glyph(ch), "expected DejaVu coverage for {ch}");
        }
    ...
    #[test]
    fn paired_arrow_aliases_map_to_covered_unicode_glyphs() {
        let font = font();
        for ch in ['⇇', '⇉', '⇆', '⇄', '⇈', '⇊', '⇅', '⇵'] {
            assert!(font.has_glyph(ch), "expected DejaVu coverage for {ch}");
        }
    ...
    #[test]
    fn vertical_harpoon_aliases_map_to_covered_unicode_glyphs() {
        let font = font();
        for ch in ['↿', '↾', '⇃', '⇂'] {
            assert!(font.has_glyph(ch), "expected DejaVu coverage for {ch}");
        }
    ...
    #[test]
    fn curved_arrow_aliases_map_to_covered_unicode_glyphs() {
        let font = font();
        for ch in ['⇠', '⇢', '↶', '↷', '↺', '↻', '⇚', '⇛', '↜'] {
            assert!(font.has_glyph(ch), "expected DejaVu coverage for {ch}");
        }
    ...
    #[test]
    fn relation_variant_aliases_map_to_covered_unicode_glyphs() {
        let font = font();
        for ch in ['≏', '≎', '≑', '≒', '≓', '≖', '≗', '≜', '≬', '⋔'] {
            assert!(font.has_glyph(ch), "expected DejaVu coverage for {ch}");
        }
        ...
        let layout = layout_math(
            concat!(
                "\\bumpeq\\Bumpeq\\doteqdot\\fallingdotseq\\risingdotseq",
                "\\eqcirc\\circeq\\triangleq\\between\\pitchfork"
            ),
            &font,
            20.0,
        );
    ...
    #[test]
    fn negated_double_arrow_aliases_map_to_covered_unicode_glyphs() {
        let font = font();
        for ch in ['⇏', '⇍', '⇎'] {
            assert!(font.has_glyph(ch), "expected DejaVu coverage for {ch}");
        }

        let layout = layout_math("\\nRightarrow\\nLeftarrow\\nLeftrightarrow", &font, 20.0);
        let text: String = layout
            .elements
            .iter()
            .filter_map(|element| match element {
    ...
    #[test]
    fn operatornamewithlimits_preserves_literal_text_and_takes_scripts() {
        let layout = layout_math("\\operatornamewithlimits{arg\\ max}_{x}", &font(), 20.0);
        let plain = layout_math("\\operatornamewithlimits{arg\\ max}", &font(), 20.0);
        let text: String = layout
            .elements
            .iter()
            .filter_map(|element| match element {
    ...
    #[test]
    fn mathregular_and_mathdefault_alias_roman_text() {
        let layout = layout_math("\\mathregular{abc}+\\mathdefault{XYZ}_0", &font(), 20.0);
        let text: String = layout
            .elements
            .iter()
            .filter_map(|element| match element {
    ...
    #[test]
    fn operatornamewithlimits_missing_argument_warns_and_preserves_command() {
        let layout = layout_math("\\operatornamewithlimits+x", &font(), 20.0);
    ...
    #[test]
    fn mathregular_missing_argument_warns_and_preserves_command() {
        let layout = layout_math("\\mathregular+x", &font(), 20.0);
        ...
        assert!(layout.elements.iter().any(
            |element| matches!(element, MathElement::Glyph { text, .. } if text == "\\mathregular")
        ));
    }
