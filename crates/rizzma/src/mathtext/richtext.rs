//! Math-aware single-line text layout.
//!
//! This module is the shared seam between ordinary text rendering and the
//! [`crate::mathtext::layout_math`] engine. It parses a label into
//! [`crate::text::TextRun`] spans, lays out plain spans with
//! [`FontSource::text_to_path`](crate::text::FontSource::text_to_path) and math
//! spans with [`crate::mathtext::layout_math`], and concatenates the resulting glyph/rule
//! [`Path`]s into a single baseline-relative frame.
//!
//! Coordinates are **y-up** with the baseline at `y = 0` and the pen starting at
//! `x = 0`, matching the figure and axis drawing coordinate system. A pure-plain
//! label produces exactly the same geometry and width as
//! [`FontSource::text_to_path`](crate::text::FontSource::text_to_path) /
//! [`FontSource::measure`](crate::text::FontSource::measure), so non-math text
//! remains unchanged.

use crate::core::Path;
use crate::text::{FontSource, TextRun, TextSpanKind};

use crate::mathtext::layout_math;
...
/// [`FontSource::text_to_path`](crate::text::FontSource::text_to_path) and math
/// spans with [`crate::mathtext::layout_math`]. Spans are placed left to right, advancing a
...
/// [`FontSource::measure`](crate::text::FontSource::measure); an empty string
/// yields an empty layout.
///
/// # Examples
///
/// ```
/// use rizzma::mathtext::layout_rich_text;
/// use rizzma::text::FontSource;
///
